...
public class VentaDao {
    ...
    public void pdfAllSales() {
    try {
        Date date = new Date();
        FileOutputStream archivo;
        String url = FileSystemView.getFileSystemView().getDefaultDirectory().getPath();
        File salida = new File(url + "ventas_detalladas.pdf");
        archivo = new FileOutputStream(salida);
        Document doc = new Document();
        PdfWriter.getInstance(doc, archivo);
        doc.open();
        ...
        // Agregar encabezado
        doc.add(new Paragraph("Lista Detallada de Ventas", new Font(Font.FontFamily.TIMES_ROMAN, 18, Font.BOLD, BaseColor.BLUE)));
        ...
        String ventasQuery = "SELECT v.id, c.nombre, v.fecha, v.total FROM ventas v INNER JOIN clientes c ON v.cliente = c.id";
        try (Connection con = cn.getConnection(); PreparedStatement ps = con.prepareStatement(ventasQuery); ResultSet rs = ps.executeQuery()) {
            while (rs.next()) {
                ...
                //tablaEncabezados.addCell(new SimpleDateFormat("dd/MM/yyyy").format(rs.getDate("fecha")));
